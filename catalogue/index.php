...
if(empty($_REQUEST['type'])) {
	header("Location: index.php?type=fa&p=1");
} else {
...
if(!empty($_REQUEST['c'])) {
	...
	if($cResult->num_rows == 0) {
		header("Location: index.php?type=".$mysqli->real_escape_string($_REQUEST['type'])."&p=1");
	} else {
...
if(!empty($_REQUEST['s'])) {
	...
	if($sResult->num_rows == 0) {
		header("Location: index.php?type=".$mysqli->real_escape_string($_REQUEST['type'])."&c=".$mysqli->real_escape_string($_REQUEST['c'])."&p=1");
	} else {
...
if(!empty($_REQUEST['s2']) and empty($_REQUEST['s'])) {
	header("Location: index.php?type=".$mysqli->real_escape_string($_REQUEST['type'])."&c=".$mysqli->real_escape_string($_REQUEST['c'])."&s=".$mysqli->real_escape_string($_REQUEST['s'])."&p=1");
}
...
<head>
    ...
    <meta name='description' content='В нашем интернет-магазине более 5.000 видов позиций мебельной фурнитуры, крепежа, кромочных материалов, ручного инструмента и прочих сопутствующих товаров. Бесплатная доставка по Беларуси.'>

    <title>Аргос-ФМ | Каталог мебельной фурнитуры</title>
	...
	<div id="page">
		...
		<?php
			...
			echo "
				...
				<div id='breadCrumbs'>
				<a href='/'><span class='breadCrumbsText'>Главная</span></a> > <a href='index.php?type=fa&p=1'><span class='breadCrumbsText'>Каталог</span></a> > <a href='index.php?type=".$type['catalogue_type']."&p=1'><span class='breadCrumbsText'>".$type['type_name']."</span></a>
			";
			...
			if(!empty($_REQUEST['c'])) {
				...
				echo " > <a href='index.php?type=" . $type['catalogue_type'] . "&c=" . $category['id'] . "&p=1'><span class='breadCrumbsText'>" . $category['name'] . "</span></a>";
			}
			...
			if(!empty($_REQUEST['s'])) {
				...
				echo " > <a href='index.php?type=".$type['catalogue_type']."&c=".$category['id']."&s=".$subcategory['id']."&p=1'><span class='breadCrumbsText'>".$subcategory['name']."</span></a>";
			}
			...
			if(!empty($_REQUEST['s2'])) {
				...
				echo " > <a href='index.php?type=".$type['catalogue_type']."&c=".$category['id']."&s=".$subcategory['id']."&s2=".$subcategory2['id']."&p=1'><span class='breadCrumbsText'>".$subcategory2['name']."</span></a>";
			}
                        ...
                        if($_REQUEST['p'] >= 5 and $_REQUEST['p'] < $check) {
                            echo "
                                        ...
                                        <div id='pageNumbers1'>
                                            ...
                                            <a href='".$link."1' class='noBorder'><div id='pb11' class='pageNumberBlock' onmouseover='pageBlock(1, \"pb11\", \"pbt11\")' onmouseout='pageBlock(0, \"pb11\", \"pbt11\")'><span class='goodStyleRed' id='pbt11'>1</span></div></a>
                                            <div class='pageNumberBlock' style='cursor: url(\"img/cursor/no.cur\"), auto;'><span class='goodStyle'>...</span></div>
                                            <a href='".$link.($_REQUEST['p'] - 1)."' class='noBorder'><div id='pb1".($_REQUEST['p'] - 1)."' class='pageNumberBlock' onmouseover='pageBlock(1, \"pb1".($_REQUEST['p'] - 1)."\", \"pbt1".($_REQUEST['p'] - 1)."\")' onmouseout='pageBlock(0, \"pb1".($_REQUEST['p'] - 1)."\", \"pbt1".($_REQUEST['p'] - 1)."\")'><span class='goodStyleRed' id='pbt1".($_REQUEST['p'] - 1)."'>".($_REQUEST['p'] - 1)."</span></div></a>
                                            <div class='pageNumberBlockActive'><span class='goodStyleWhite'>".$_REQUEST['p']."</span></div>
                                            <a href='".$link.($_REQUEST['p'] + 1)."' class='noBorder'><div id='pb1".($_REQUEST['p'] + 1)."' class='pageNumberBlock' onmouseover='pageBlock(1, \"pb1".($_REQUEST['p'] + 1)."\", \"pbt1".($_REQUEST['p'] + 1)."\")' onmouseout='pageBlock(0, \"pb1".($_REQUEST['p'] + 1)."\", \"pbt1".($_REQUEST['p'] + 1)."\")'><span class='goodStyleRed' id='pbt1".($_REQUEST['p'] + 1)."'>".($_REQUEST['p'] + 1)."</span></div></a>
                                            <div class='pageNumberBlock' style='cursor: url(\"img/cursor/no.cur\"), auto;'><span class='goodStyle'>...</span></div>
                                            <a href='".$link.$numbers."' class='noBorder'><div id='pb1".$numbers."' class='pageNumberBlock' onmouseover='pageBlock(1, \"pb1".$numbers."\", \"pbt1".$numbers."\")' onmouseout='pageBlock(0, \"pb1".$numbers."\", \"pbt1".$numbers."\")'><span class='goodStyleRed' id='pbt1".$numbers."'>".$numbers."</span></div></a>
