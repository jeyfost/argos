...
if($searchResult->num_rows == 0) {
    ...
    while($search = $searchResult->fetch_assoc()) {
        ...
        if(($user['opt'] == 1 and ($search['price'] == 0 or $search['price'] == null or $search['price_opt'] == 0)) or ($user['opt'] == 0 and ($search['price'] == 0 or $search['price'] == null))) {
            ...
            if($active == 0) {
                ...
                if(isset($_SESSION['userID'])) {
                    $discountResult = $mysqli->query("SELECT discount FROM users WHERE id = '".$user_id."'");
        ...
        echo "
			<div class='searchItem'"; if($j % 2 == 0) {echo " style='background-color: #d9d9d9;'";} echo " onclick='chooseGood(\"".$search['id']."\", \"g_".substr($id, 7)."\", \"".$user_id."\", \"".$order_id."\")'>
				<div class='searchIMG'>
        ...
        if($active == 0) {
            echo "
			    <b>Цена "; if($user['opt'] == 1) {echo "оптовая";} else {echo "розничная";} echo " за ".$unit['short_name']; if($discount[0] > 0) {echo " с учётом скидки";} echo ": </b>".$price."
            ";
        } else {
            $price = $actionGood['price'] * $rate[0];
            ...
            echo "
			        <b>Цена акционная за ".$unit['short_name']; if($discount[0] > 0) {echo " с учётом скидки";} echo ": </b><span style='color: #ff282b; font-weight: bold;'>".$price."</span>
                ";
        }
