...
if(isset($_REQUEST['album'])) {
	$albumCheckResult = $mysqli->query("SELECT * FROM albums WHERE id = '".$mysqli->real_escape_string($_REQUEST['album'])."'");
...
<body style="background-color: #e4e4e4;">
	...
	<div id="content">
		...
		<br />
		<div id="admContent">
			<div id="breadCrumbs"><div id="breadCrumbsIcon"><img src="/img/system/admin/icons/photo.png" title="Фотогалерея" /></div><div id="breadCrumbsTextContainer"><a href="/admin/admin.php"><span class="breadCrumbsText">Панель администрирования</span></a> > <a href="index.php"><span class="breadCrumbsText">Фотогалерея</span></a> > <a href="delete.php"><span class="breadCrumbsText">Удаление фотографий</span></a></div></div>
			<div style="clear: both;"></div>
			<br />
				...
				<select id="albumSelect" name="album" onchange="window.location = 'delete.php?album=' + this.options[this.selectedIndex].value">
					...
					<?php
						...
						while($album = $albumResult->fetch_assoc()) {
							echo "<option value='".$album['id']."'"; if($_REQUEST['album'] == $album['id']) {echo " selected";} echo ">".$album['name']."</option>";
						}
					?>
				</select>
				<?php
