...
if(!empty($_REQUEST['id'])) {
    $colorCheckResult = $mysqli->query("SELECT COUNT(id) FROM handles_colors WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
...
<head>
    ...
    <title>Редактирование цветов ручек</title>
    ...
    <script type="text/javascript" src="/js/admin/filters/colors/edit.js"></script>
...
<div id="content">
    ...
    <div id="admContent">
        ...
        <form id="editForm" method="post">
            <label for="colorSelect">Выберите цвет ручек:</label>
            <br />
            <select id="colorSelect" name="color" onchange="window.location = 'edit.php?id=' + this.options[this.selectedIndex].value">
                <option value="">- Выберите цвет ручек -</option>
                <?php
                    $colorResult = $mysqli->query("SELECT * FROM handles_colors ORDER BY name");
                    while ($color = $colorResult->fetch_assoc()) {
                        echo "
                            <option value='".$color['id']."'"; if($_REQUEST['id'] == $color['id']) {echo " selected";} echo ">".$color['name']."</option>
                            ";
                    }
                ?>
            </select>
            <?php
            if(!empty($_REQUEST['id'])) {
                $colorResult = $mysqli->query("SELECT * FROM handles_colors WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
                $color = $colorResult->fetch_assoc();

                echo "
                                <br /><br />
                                <label for='colorInput'>Цвет ручек:</label>
                                <br />
                                <input type='text' id='colorInput' name='color' value='".$color['name']."' />
                                <br /><br />
                                <input type='button' class='button' style='margin: 0' id='editColorButton' onmouseover='buttonChange(\"editColorButton\", 1)' onmouseout='buttonChange(\"editColorButton\", 0)' onclick='editColor()' value='Редактировать' />
                            ";
            }
            ?>
        </form>
