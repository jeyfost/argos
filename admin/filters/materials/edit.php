...
if(!empty($_REQUEST['id'])) {
    $materialCheckResult = $mysqli->query("SELECT COUNT(id) FROM handles_materials WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
...
<head>
    ...
    <title>Редактирование материалов ручек</title>
    ...
    <script type="text/javascript" src="/js/admin/filters/materials/edit.js"></script>
...
<div id="content">
    ...
    <div id="admContent">
        ...
        <div style="clear: both;"></div>
        <br />
        <h2>Редактирование материалов ручек</h2>
        ...
        <form id="editForm" method="post">
            <label for="materialSelect">Выберите материал ручек:</label>
            <br />
            <select id="materialSelect" name="material" onchange="window.location = 'edit.php?id=' + this.options[this.selectedIndex].value">
                <option value="">- Выберите материал ручек -</option>
                <?php
                    $materialResult = $mysqli->query("SELECT * FROM handles_materials ORDER BY name");
                    while ($material = $materialResult->fetch_assoc()) {
                        echo "
                            <option value='".$material['id']."'"; if($_REQUEST['id'] == $material['id']) {echo " selected";} echo ">".$material['name']."</option>
                            ";
                    }
                ?>
            </select>
            <?php
            if(!empty($_REQUEST['id'])) {
                $materialResult = $mysqli->query("SELECT * FROM handles_materials WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
                $material = $materialResult->fetch_assoc();

                echo "
                                <br /><br />
                                <label for='materialInput'>Материал ручек:</label>
                                <br />
                                <input type='text' id='materialInput' name='material' value='".$material['name']."' />
                                <br /><br />
                                <input type='button' class='button' style='margin: 0' id='editMaterialButton' onmouseover='buttonChange(\"editMaterialButton\", 1)' onmouseout='buttonChange(\"editMaterialButton\", 0)' onclick='editMaterial()' value='Редактировать' />
                            ";
            }
            ?>
        </form>
