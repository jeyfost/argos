...
if(!empty($_REQUEST['id'])) {
    $typeCheckResult = $mysqli->query("SELECT COUNT(id) FROM handles_types WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
    $typeCount = $typeCheckResult->fetch_array(MYSQLI_NUM);
...
<html>
    ...
    <head>
        ...
        <title>Редактирование типа ручек</title>
        ...
        <script type="text/javascript" src="/js/admin/filters/types/edit.js"></script>
    ...
    <div id="content">
        ...
        <div id="admContent">
            ...
            <form id="editForm" method="post">
                <label for="typeSelect">Выберите тип ручек:</label>
                <br />
                <select id="typeSelect" name="type" onchange="window.location = 'edit.php?id=' + this.options[this.selectedIndex].value">
                    <option value="">- Выберите тип ручек -</option>
                    <?php
                    $typeResult = $mysqli->query("SELECT * FROM handles_types ORDER BY name");
                    while ($type = $typeResult->fetch_assoc()) {
                        echo "
                                    <option value='".$type['id']."'"; if($_REQUEST['id'] == $type['id']) {echo " selected";} echo ">".$type['name']."</option>
                                ";
                    }
                    ?>
                </select>
                <?php
                if(!empty($_REQUEST['id'])) {
                    $typeResult = $mysqli->query("SELECT * FROM handles_types WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
                    $type = $typeResult->fetch_assoc();

                    echo "
                                <br /><br />
                                <label for='typeInput'>Название типа ручек:</label>
                                <br />
                                <input type='text' id='typeInput' name='type' value='".$type['name']."' />
                                <br /><br />
                                <input type='button' class='button' style='margin: 0' id='editTypeButton' onmouseover='buttonChange(\"editTypeButton\", 1)' onmouseout='buttonChange(\"editTypeButton\", 0)' onclick='editType()' value='Редактировать' />
                            ";
                }
                ?>
            </form>
