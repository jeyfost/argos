...
if(!empty($_REQUEST['id'])) {
    $brandCountResult = $mysqli->query("SELECT COUNT(id) FROM handles_brands WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
    $brandCount = $brandCountResult->fetch_array(MYSQLI_NUM);
...
<head>
    ...
    <title>Редактирование бренда</title>
    ...
    <script type="text/javascript" src="/js/admin/filters/brands/edit.js"></script>
    ...
    <div id="content">
        ...
        <div id="admContent">
            ...
            <form id="editForm" method="post">
                ...
                <br />
                <select id="brandSelect" name="brand" onchange="window.location = 'edit.php?id=' + this.options[this.selectedIndex].value">
                    <option value="">- Выберите бренд -</option>
                    <?php
                        $brandResult = $mysqli->query("SELECT * FROM handles_brands ORDER BY name");
                        while ($brand = $brandResult->fetch_assoc()) {
                            echo "
                                <option value='".$brand['id']."'"; if($_REQUEST['id'] == $brand['id']) {echo " selected";} echo ">".$brand['name']."</option>
                            ";
                        }
                    ?>
                </select>
                <?php
                    if(!empty($_REQUEST['id'])) {
                        $brandResult = $mysqli->query("SELECT * FROM handles_brands WHERE id = '".$mysqli->real_escape_string($_REQUEST['id'])."'");
                        ...
                        echo "
                            ...
                            <br />
                            <input type='text' id='nameInput' name='name' value='".$brand['name']."' />
                            <br /><br />
                            <input type='button' class='button' style='margin: 0' id='editBrandButton' onmouseover='buttonChange(\"editBrandButton\", 1)' onmouseout='buttonChange(\"editBrandButton\", 0)' onclick='editBrand()' value='Редактировать' />
                        ";
                    }
                ?>
            </form>
