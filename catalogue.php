						...
						<div class='itemDescription'>
							...
							echo "
								...
								<div id='goodPrice".$catalogue['id']."'>
									<span"; if($_SESSION['userID'] == 1 and $active == 0) {echo " style='cursor: pointer;' onclick='changePrice(\"".$catalogue['id']."\", \"goodPrice".$catalogue['id']."\", \"".$catalogue['price']."\", \"".$currency['code']."\", \"".$unit['short_name']."\", \"".$currency['rate']."\")' title='Изменить стоимость товара'";} echo "><b>Цена за ".$unit['for_name'].": </b>"; if(($user['opt'] == 1 and ($catalogue['price'] == 0 or $catalogue['price'] == null or $catalogue['price_opt'] == 0)) or ($user['opt'] == 0 and ($catalogue['price'] == 0 or $catalogue['price'] == null))) {echo "по запросу";} else {if($active > 0) {echo "<span style='color: #ff282b; font-weight: bold;'>";} if($roubles > 0) {echo $roubles." руб. ";} echo ceil($kopeck)." коп.</span>"; if($active > 0) {echo "</span>";}} echo "
								</div>
							";
